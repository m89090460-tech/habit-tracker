...
import { fetchCheckins, upsertCheckin } from "./lib/growthApi";
...
import GrowthCheckin from "./components/GrowthCheckin";
import GrowthChart from "./components/GrowthChart";
import { Plus, Sun, MoonStar, Download, Sparkles } from "lucide-react";
...
function App() {
	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				const ci = await fetchCheckins(userId);
				const ids = await fetchIdentities(userId);
				setIdentities(ids);
				setHabits(h);
				setHistory(hist);
				setCheckins(ci);
	...
	const submitCheckin = async ({ answers, direction }) => {
		const userId = session.user.id;
		const date = todayStr();
		try {
			await upsertCheckin(userId, date, todayScore, answers, direction);
			setCheckins((prev) => {
				const filtered = prev.filter((c) => c.date !== date);
				return [...filtered, { date, direction, habit_score: todayScore }];
			});
			setShowCheckin(false);
		} catch (e) {
			setToast("Sharhni saqlashda xatolik yuz berdi.");
		}
	};
	const addIdentity = async (name, description) => {
		try {
			const created = await insertIdentity(session.user.id, name, description);
			setIdentities((prev) => [...prev, created]);
		} catch (e) {
			setToast("Identity qo'shishda xatolik yuz berdi.");
		}
	};

	const removeIdentity = async (id) => {
		try {
			await deleteIdentity(session.user.id, id);
			setIdentities((prev) => prev.filter((i) => i.id !== id));
		} catch (e) {
			setToast("Identity o'chirishda xatolik yuz berdi.");
		}
	};
	...
	const votes = computeVotes(habits, history);
					...
					<>
									...
									<div className="flex items-center gap-5 sm:gap-6 mb-5">
										...
										<div>
											...
											<button
												...
												className={`mt-2 flex items-center gap-1.5 text-xs font-medium rounded-full px-3 py-1.5 w-fit transition-colors ${t.accent} disabled:opacity-40 disabled:cursor-not-allowed`}>
												<Sparkles size={13} />
												{todayCheckin ? "Bugun sharh berildi" : "Kunlik sharh"}
											</button>
										</div>
									</div>
									...
									<div className="flex items-end gap-1.5 sm:gap-2 h-16">
										{weekData.map((d) => (
											<div
												...
												<span
													className={`text-[9px] sm:text-[10px] ${t.muted}`}>
													{d.label}
												</span>
											</div>
										))}
						...
						{tab === "stats" && (
							<div className="lg:max-w-2xl space-y-6">
								<GrowthChart checkins={checkins} t={t} />
