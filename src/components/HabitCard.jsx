import { useRef, useState, useEffect } from "react";
import { motion, Reorder } from "framer-motion";
import { ICONS, COLORS } from "../lib/constants";
import { isChainBroken } from "../lib/streakGuard";
import {
	Plus,
	Minus,
	Pencil,
	Trash2,
	Bell,
	GripVertical,
	AlertTriangle,
	FileSignature,
} from "lucide-react";
import Confetti from "./Confetti";
import { Sparkle } from "lucide-react";

function HabitCard({
	habit,
	history,
	identities = [],
	onBump,
	onEdit,
	onDelete,
	t,
}) {
	const Icon = ICONS[habit.icon];
	const color = COLORS[habit.color];
	const percent = Math.min(
		100,
		Math.round((habit.current / habit.target) * 100),
	);
	const done = habit.current >= habit.target;
	const chainBroken = history ? isChainBroken(habit, history) : false;
	const hasContract = Boolean(habit.contractSignedAt);
	const hasLoop = habit.cue || habit.craving || habit.response || habit.reward;
	const identity = identities.find((i) => i.id === habit.identityId);
	const prevDone = useRef(done);
	const [burst, setBurst] = useState(false);
	const [showLoop, setShowLoop] = useState(false);

	useEffect(() => {
		if (done && !prevDone.current) {
			setBurst(true);
			const timer = setTimeout(() => setBurst(false), 700);
			return () => clearTimeout(timer);
		}
		prevDone.current = done;
	}, [done]);

	return (
		<Reorder.Item
			value={habit}
			initial={{ opacity: 0, y: 10 }}
			animate={{ opacity: 1, y: 0 }}
			exit={{ opacity: 0, x: -20 }}
			transition={{ duration: 0.3 }}
			whileDrag={{ scale: 1.03, boxShadow: "0 10px 30px rgba(0,0,0,0.3)" }}
			className={`relative group rounded-[24px] border p-4 sm:p-5 mb-3 ${t.card}`}>
			{burst && <Confetti />}

			{chainBroken && (
				<div className="flex items-center gap-1.5 text-[11px] text-rose-400 bg-rose-500/10 rounded-lg px-2.5 py-1.5 mb-3">
					<AlertTriangle size={12} /> Zanjir 2 kun uzilmoqda — bugun buzmang!
				</div>
			)}

			<div className="flex items-center gap-3 sm:gap-4">
				<GripVertical
					size={15}
					className={`${t.muted} cursor-grab active:cursor-grabbing shrink-0`}
				/>

				<div
					className="w-10 h-10 sm:w-11 sm:h-11 rounded-2xl flex items-center justify-center shrink-0"
					style={{ background: color.soft }}>
					<Icon size={18} style={{ color: color.ring }} />
				</div>

				<div className="flex-1 min-w-0">
					<div className="flex items-start justify-between gap-2">
						<button
							onClick={() => onEdit(habit)}
							className="flex-1 min-w-0 text-left">
							<p className={`text-sm font-medium truncate ${t.strong}`}>
								{habit.name}
							</p>
							<div className="flex items-center gap-2 mt-0.5 flex-wrap">
								{habit.reminderTime && (
									<span
										className={`text-[10px] flex items-center gap-1 ${t.muted}`}>
										<Bell size={9} /> {habit.reminderTime}
									</span>
								)}
								{hasContract && (
									<span className="text-[10px] flex items-center gap-1 text-violet-400">
										<FileSignature size={9} /> Shartnoma
									</span>
								)}
								{identity && (
									<span className="text-[10px] flex items-center gap-1 text-amber-400">
										<Sparkle size={9} /> {identity.name}
									</span>
								)}
							</div>
						</button>
						<div className="flex items-center gap-0.5 opacity-70 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity shrink-0">
							<button
								onClick={() => onEdit(habit)}
								className={`${t.muted} hover:text-violet-400 p-1.5`}>
								<Pencil size={14} />
							</button>
							<button
								onClick={() => onDelete(habit)}
								className={`${t.muted} hover:text-rose-400 p-1.5`}>
								<Trash2 size={14} />
							</button>
						</div>
					</div>
					<div
						className={`w-full h-2 rounded-full mt-2 overflow-hidden ${t.track}`}>
						<div
							className={`h-full rounded-full bg-gradient-to-r ${color.bar}`}
							style={{
								width: `${percent}%`,
								transition: "width 0.4s ease-out",
							}}
						/>
					</div>
				</div>

				<div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
					<button
						onClick={() => onBump(habit.id, -1)}
						className={`w-7 h-7 rounded-full flex items-center justify-center ${t.track} ${t.strong}`}>
						<Minus size={13} />
					</button>
					<span
						className={`text-sm font-semibold w-10 sm:w-14 text-center ${t.strong}`}>
						{habit.current}/{habit.target}
					</span>
					<button
						onClick={() => onBump(habit.id, 1)}
						className="w-7 h-7 rounded-full flex items-center justify-center text-white"
						style={{ background: color.ring }}>
						<Plus size={13} />
					</button>
				</div>
			</div>

			{done && (
				<p className="text-[11px] text-emerald-400 mt-2">✓ Bajarildi!</p>
			)}

			{hasLoop && (
				<div className="mt-2">
					<button
						onClick={() => setShowLoop((s) => !s)}
						className={`text-[10px] ${t.muted} hover:${t.strong}`}>
						{showLoop ? "▲ Sikldan yopish" : "▼ Odat siklini ko'rish"}
					</button>
					{showLoop && (
						<div
							className={`mt-2 rounded-xl p-3 text-[11px] space-y-1 ${t.track}`}>
							{habit.cue && (
								<p className={t.sub}>
									<b className={t.strong}>Signal:</b> {habit.cue}
								</p>
							)}
							{habit.craving && (
								<p className={t.sub}>
									<b className={t.strong}>Istak:</b> {habit.craving}
								</p>
							)}
							{habit.response && (
								<p className={t.sub}>
									<b className={t.strong}>Javob:</b> {habit.response}
								</p>
							)}
							{habit.reward && (
								<p className={t.sub}>
									<b className={t.strong}>Mukofot:</b> {habit.reward}
								</p>
							)}
						</div>
					)}
				</div>
			)}
		</Reorder.Item>
	);
}

export default HabitCard;
